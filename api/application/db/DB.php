...
class DB {
    ...
    public function gameVictory($partyId) {
        $query = "UPDATE parties SET status='ended' WHERE id=".$partyId;
        $this->conn->query($query);
        return true;
    }
    ...
    public function getPartyByUserIdAndAi($userId, $aiID) {
        $query = 'SELECT * FROM parties WHERE (user1_id=' . $userId . ' AND user2_id=' . $aiID . ')';
        $result = $this->conn->query($query);
        return $result->fetch_object();
    }
    ...
    public function rememberTheMove($partyId, $areaX, $areaY, $x, $y, $player)
    {
        $query = 'INSERT INTO progress (party_id, area_x, area_y, x, y, player) VALUES (
            ' . $partyId . ',
            ' . $areaX. ',
            ' . $areaY . ',
            ' . $x . ',
    ...
    public function getProgress($partyId, $player)
    {
        $level = PiXO::IMMERSION_LEVEL + 1;

        $query = 'SELECT area_x, area_y, x, y FROM progress WHERE (party_id=' . $partyId . '
            AND player=' . $player . ') ORDER BY id DESC LIMIT ' . $level;
        $result = $this->conn->query($query);
